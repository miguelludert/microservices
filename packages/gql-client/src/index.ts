...
// export async function appsyncClient(props : AppsyncClientProps) {
...
//     const request = await aws.sign(process.env.GRAPHQL_URL, {
...
export async function appsyncClient<TResult = any>(props: AppsyncClientProps) {
  ...
  try {
    ...
    if(errors) {
      ...
      console.info('appsyncClient - application error: ',err);
    }
    throw err;
  }
}
...
//   const endpoint = new AWS.Endpoint(process.env.GRAPHQL_URL);
//   const { query, variables, operationName } = props;
//   const body = JSON.stringify({ query, variables, operationName });
